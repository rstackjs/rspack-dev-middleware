...
import { FilenameError } from "../src/utils/getFilenameFromUrl.js";
      ...
      describe("getFilenameFromUrl method", () => {
        describe("should work", () => {
          ...
          it("should work", async () => {
            ...
            const bundle = instance.getFilenameFromUrl("/bundle.js");

            expect(bundle.filename).toBe(
              path.join(webpackConfig.output.path, "/bundle.js"),
            );
            expect(bundle.extra.outputFileSystem).toBe(
              instance.context.outputFileSystem,
            );
            expect(bundle.extra.stats.isFile()).toBe(true);
        ...
        describe('should work with the "publicPath"', () => {
          ...
          it("should work", async () => {
            ...
            expect(instance.getFilenameFromUrl("/public/path/").filename).toBe(
            ...
            ).toBe(path.join(webpackPublicPathConfig.output.path, "/svg.svg"));
        ...
        describe("should work in multi compiler mode", () => {
          ...
          it("should work", async () => {
            ...
            ).toBe(path.join(webpackMultiConfig[0].output.path, "/bundle.js"));
            ...
            ).toBe(path.join(webpackMultiConfig[0].output.path, "/index.html"));
            expect(
            ...
            ).toBe(path.join(webpackMultiConfig[1].output.path, "/bundle.js"));
            expect(
        ...
        describe("should throw typed errors", () => {
          ...
          it("should throw for malicious paths and null bytes", async () => {
            await waitUntilValid(instance);

            try {
              instance.getFilenameFromUrl("/..%2f../middleware.test.js");
              throw new Error("Expected getFilenameFromUrl to throw");
            } catch (error) {
              expect(error).toBeInstanceOf(FilenameError);
              expect(error.statusCode).toBe(403);
            }

            try {
              instance.getFilenameFromUrl("/%00");
              throw new Error("Expected getFilenameFromUrl to throw");
            } catch (error) {
              expect(error).toBeInstanceOf(FilenameError);
...
<pre>Range Not Satisfiable</pre>
        ...
        it('should return the "206" code for the "GET" request with the valid range header', async () => {
          ...
          const rangeEnd = Math.min(3500, codeBuffer.byteLength - 1);
          const expectedRange = codeBuffer.subarray(3000, rangeEnd + 1);
          const response = await req
            .get("/bundle.js")
          ...
          expect(response.headers["content-range"]).toBe(
            `bytes 3000-${rangeEnd}/${codeBuffer.byteLength}`,
          );
          expect(response.headers["content-length"]).toBe(
            String(expectedRange.byteLength),
          );
          expect(response.headers["content-type"]).toEqual(
            getContentTypeHeader(),
          );
          expect(Buffer.from(response.text)).toEqual(expectedRange);
          expect(Buffer.byteLength(response.text)).toBe(
            expectedRange.byteLength,
          );
        });
        ...
        it('should return the "206" code for the "GET" request with the valid range header when range starts with 0', async () => {
          const codeBuffer = Buffer.from(codeContent);
          const rangeEnd = Math.min(3500, codeBuffer.byteLength - 1);
          const expectedRange = codeBuffer.subarray(0, rangeEnd + 1);
          const response = await req
            .get("/bundle.js")
          ...
          expect(response.statusCode).toBe(206);
          expect(response.headers["content-range"]).toBe(
            `bytes 0-${rangeEnd}/${codeBuffer.byteLength}`,
          );
          expect(response.headers["content-length"]).toBe(
            String(expectedRange.byteLength),
          );
...
<pre>Forbidden</pre>
      ...
      describe("should work and generate strong etag", () => {
        ...
        it('should return the "200" code for the "GET" request with the valid range header and wrong "If-Range" header', async () => {
          const response = await req
            .get("/bundle.js")
            .set("Range", "bytes=3000-3500");
          const bundleBuffer = instance.context.outputFileSystem.readFileSync(
            path.resolve(webpackConfig.output.path, "bundle.js"),
          );
          const rangeEnd = Math.min(3500, bundleBuffer.byteLength - 1);
          const expectedRange = bundleBuffer.subarray(3000, rangeEnd + 1);

          expect(response.statusCode).toBe(206);
          expect(response.headers["content-range"]).toBe(
            `bytes 3000-${rangeEnd}/${bundleBuffer.byteLength}`,
          );
          expect(response.headers["content-length"]).toBe(
            String(expectedRange.byteLength),
          );
    ...
    describe("cacheImmutable", () => {
      ...
      describe("should use cacheControl object option (with only immutable: true) when cacheImmutable is false, and add 'immutable' to Cache-Control header", () => {
        ...
        it('should return the "200" code for the "GET" request to the bundle file and generate `Cache-Control` header from cacheControl object option with immutable', async () => {
          const response = await req.get("/main.js");
        ...
        it('should return the "200" code for the "GET" request to the immutable asset and generate `Cache-Control` header from cacheControl object option with immutable', async () => {
          await req.get("/main.js");
